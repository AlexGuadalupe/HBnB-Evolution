...
amenities_blueprint = Blueprint('amenities_api', __name__)
amenities = []
...
@amenities_blueprint.route('/ammenities', methods=['GET'])
def get_amenities():
    """Get all amenities."""
    return jsonify(amenities), 200


@amenities_blueprint.route('/ammenities/{amenity_id}', methods=['GET'])
def get_amenity(amenity_id):
    """Get a specific amenity."""
    amenity = next(
        (_amenity for _amenity in amenities if _amenity["id"] == amenity_id), None)
    if amenity is None:
        return jsonify({"error": "Amenity not found."}), 404
    return jsonify(amenity), 200


@amenities_blueprint.route('/ammenities/{amenity_id}', methods=['PUT'])
...
@amenities_blueprint.route('/ammenities/{amenity_id}', methods=['DELETE'])
